...
AC_DEFUN(TEA_ENABLE_THREADS, [
    AC_ARG_ENABLE(threads, [  --disable-threads       do not build with threads [--enable-threads]],
...
AC_DEFUN(TEA_CONFIG_CFLAGS, [
	...
	#CFLAGS_WARNING="-Wall -Wconversion -Wpointer-arith -Wcast-align -Waggregate-return -Wstack-protector -Wtraditional -Wmissing-declarations -Wnested-externs -Winline -ansi -pedantic -Wno-long-long -Wmissing-prototypes -Wstrict-prototypes -Wwrite-strings"
    else
	CFLAGS_WARNING=""
    fi
...
dnl AC_CHECK_TOOL(AR, ar, :)
	...
	OpenBSD-*)
	    ...
	    *)
	    ...
	    SHLIB_LD="${CC} -shared"
	    SHLIB_LD_EXTRAS='-nostdlib'
	    SHLIB_LD_LIBS='${LIBS} -lc'
	    SHLIB_SUFFIX=".so"
	    DL_OBJS="tclLoadDl.o"
	    DL_LIBS=""
	    LDFLAGS=""
	    LDFLAGS_OPTIMIZE='-Wl,${CFLAGS_OPTIMIZE}'
	    CFLAGS_OPTIMIZE="-O2"
	    CFLAGS_WARNING="-Wall"
	    ...
	    # if test "${TCL_THREADS}" = "1" ; then
	    # 	LDFLAGS="$LDFLAGS -pthread"
	    # 	SHLIB_LDFLAGS="$SHLIB_LD_FLAGS -pthread"
	    # fi
	    SHARED_LIB_SUFFIX='\$\{DBGX\}.so.${VERSION}'
...
AC_DEFUN(TEA_MAKE_LIB, [
	...
	MAKE_SHARED_LIB="\${SHLIB_LD} -o \[$]@ \$(\[$](PACKAGE)_OBJECTS) \${SHLIB_LDFLAGS} \${SHLIB_LD_EXTRAS} \${SHLIB_LD_LIBS}"
...
AC_DEFUN(TEA_PUBLIC_TCL_HEADERS, [
	...
	if test x"${with_tclinclude}" != x ; then
		...
		TCL_INCLUDE_DIR=`echo $TCL_INCLUDE_SPEC | sed -e 's/^ *-I//'`
		if test -f "${TCL_INCLUDE_DIR}/tcl.h" ; then
		    ac_cv_c_tclh=${TCL_INCLUDE_DIR}
		fi
	    fi
...
AC_DEFUN(TEA_PUBLIC_TK_HEADERS, [
	...
	if test x"${with_tkinclude}" != x ; then
		...
		if test -f "${TK_INCLUDE_DIR}/tk.h" ; then
		    ac_cv_c_tkh=${TK_INCLUDE_DIR}
		else
		    AC_MSG_ERROR([${TK_INCLUDE_DIR} directory does not contain tk.h])
		fi
	    else
		# Check order: pkg --prefix location, Tcl's --prefix location,
		# directory of tclConfig.sh, and Tcl source directory.
